...
//        print("MOVE x: \(movedX), y: \(movedY)")
    ...
    private static func getCursor(_ name: String) -> NSCursor {
        guard let bundle = Bundle(for: self).resourceURL else {
                print("Failed to retrieve bundle URL.")
            return NSCursor.arrow
            }
            
            let plistURL = bundle.appendingPathComponent("resources/\(name)_info.plist")
            
            guard let infoDict = NSDictionary(contentsOf: plistURL) else {
                print("Failed to load info plist for cursor: \(name)")
                return NSCursor.arrow
            }
        
        let imagePath = bundle.appendingPathComponent("resources/\(name)_cursor.pdf").path
        
            guard let image = NSImage(byReferencingFile: imagePath) else {
                print("Failed to load cursor image for: \(name)")
                return NSCursor.arrow
            }
            
            guard let hotX = infoDict["hotx"] as? Double,
                  let hotY = infoDict["hoty"] as? Double else {
                print("Missing hot spot coordinates in plist for cursor: \(name)")
                return NSCursor.arrow
            }
